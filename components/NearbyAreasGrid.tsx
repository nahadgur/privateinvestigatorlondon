...
export function NearbyAreasGrid({ cityName, serviceSlug, serviceName, initialVisible = 10 }: NearbyAreasGridProps) {
  ...
  const description = serviceName
    ? `Looking for ${serviceName.toLowerCase()} near ${cityName}? Our vetted investigators serve clients across ${cityName} and surrounding areas. We will match you with the right investigator.`
    : `Our investigators serve clients from across the surrounding area. If you are based in any of the nearby areas listed below, we can match you with the right specialist.`;
  ...
  return (
    <section className="mb-16">
      ...
      <div className="mt-6 prose prose-sm max-w-none text-gray-500">
        <p>
          Clients from {areas.slice(0, 5).join(', ')}, and other areas around {cityName} regularly use our service to find private investigators.{' '}
          {serviceName
            ? `If you need ${serviceName.toLowerCase()} and are based in or near ${cityName}, our vetted private investigators can arrange a free initial conversation at a time that suits you.`
            : `All of our ${cityName} partner private investigators are vetted, insured, and responsive to your timeline and project needs.`
          }
        </p>
      </div>
    </section>
