...
export function PricingSection({ cityName, serviceId, serviceName }: PricingSectionProps) {
  ...
  const heading = cityName && serviceName
    ...
    : cityName
    ? `Private Investigator Costs in ${cityName}`
    : serviceName
    ? `${serviceName} Pricing Guide`
    : 'Pricing Guide';

  const intro = cityName
    ? `Fees in ${cityName} vary depending on the investigation type and case complexity. Below are typical costs from vetted investigators in the ${cityName} area. All prices are in GBP.`
    : 'Fees vary depending on the investigation type and case complexity. Below are typical costs from investigators in our network. All prices are in GBP.';
    ...
    <section className="mb-16">
      ...
      {cityName && (
        <div className="mt-8 prose prose-sm max-w-none text-gray-600">
          <p>
            Private investigator costs in {cityName} vary by case type and complexity. Investigators in our network provide clear proposals before any work begins.
          </p>
        </div>
      )}
    </section>
