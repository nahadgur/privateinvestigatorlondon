  ...
  if (isSuccess) {
    return (
      <div className="bg-white text-gray-900 rounded-2xl p-8 shadow-2xl border border-gray-100 flex flex-col items-center justify-center text-center gap-4 min-h-[340px]">
        ...
        <h3 className="text-2xl font-display font-bold">Enquiry Received</h3>
        <p className="text-gray-600">
          We will match you with a vetted private investigator{city ? ` in ${city}` : ' in London'}. Check your email for next steps.
        </p>
      </div>
    );
  }
    ...
    <div className="bg-white text-gray-900 rounded-2xl p-6 md:p-8 shadow-2xl border border-gray-100">
      <div className="mb-6">
        ...
        <h3 className="text-2xl font-display font-bold leading-tight">
          Describe Your Case{city ? ` in ${city}` : ''}
        </h3>
        <p className="text-gray-600 text-sm mt-1">
          A vetted private investigator will contact you within 24 hours
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        ...
        <select required name="treatment" value={formData.treatment} onChange={handleChange} className={inputClass + " appearance-none cursor-pointer"}>
          <option value="" disabled>Type of investigation *</option>
          {services.map(s => (
        ...
        {!city && (
          <input required name="location" type="text" value={formData.location} onChange={handleChange} placeholder="Area or postcode *" className={inputClass} />
        )}
        ...
        >
          {isSubmitting ? 'Sending...' : 'Get a Free Match'}
        </button>

        <div className="flex items-center justify-center gap-4 pt-1">
          {['100% Confidential', 'No Obligation', '24hr Response'].map(item => (
