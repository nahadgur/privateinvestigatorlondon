...
export default function ServiceLocationPage({ params }: { params: Promise<{ serviceSlug: string; locationSlug: string }> }) {
  ...
  const benefits = [
    { icon: <Award className="w-6 h-6" />, title: 'Vetted Private Investigators', desc: `Every private investigator in our network has been vetted on credentials and operational experience before receiving any referrals.` },
    { icon: <Clock className="w-6 h-6" />, title: 'Response Within 24 Hours', desc: `Most private investigators in our network respond within 24 hours and can arrange a confidential initial conversation to discuss your case.` },
    { icon: <Shield className="w-6 h-6" />, title: 'Vetted and Insured', desc: `All investigators in our network operate within UK law and are assessed on credentials and experience before joining.` },
    { icon: <Users className="w-6 h-6" />, title: 'Matched to Your Brief', desc: `We connect you with investigators who have specific experience with your case type — not a generic list of whoever is available.` },
  ];
  ...
  const localBusinessSchema = {
    ...
    description: `Find vetted private investigators in ${cityName}. Confidential matching service, no obligation.`,
    areaServed: {
      '@type': 'City',
      name: cityName,
      ...
      <main className="flex-grow">
              ...
              <div>
                ...
                <div className="space-y-4 mb-8">
                  {[
                    ...
                    'Up to three matched private investigators, each providing a confidential initial case review',
                    'Vetted, credentialled, and legally compliant investigators',
                  ].map((item, i) => (
                ...
                <div className="flex items-center gap-4 text-sm text-gray-400">
                  ...
                  <span>Trusted by London clients</span>
                </div>
              </div>
              <div>
        ...
        <div className="container-width py-16">
            ...
            <aside className="lg:col-span-1">
              <div className="sticky top-28 space-y-8">
                ...
                <div className="bg-brand-900 text-white p-6 rounded-2xl shadow-lg">
                  ...
                  <p className="text-brand-100 text-sm mb-4">Most investigators require a retainer before commencing work, with the balance payable on delivery of the completed report.</p>
                  <button onClick={() => setIsModalOpen(true)} className="block w-full bg-white text-brand-900 text-center font-bold py-3 px-6 rounded-xl hover:bg-brand-50 transition-colors text-sm">Get a Free Match</button>
                </div>
              </div>
            </aside>
          </div>
          ...
          <div className="bg-brand-900 rounded-2xl p-8 md:p-12 text-center mt-12">
            ...
            <p className="text-brand-200 mb-8 max-w-2xl mx-auto">Describe your case in under two minutes. We will match you with up to three vetted {cityName} investigators with no obligation at any stage.</p>
            <button onClick={() => setIsModalOpen(true)} className="bg-white text-brand-900 font-bold text-lg py-4 px-10 rounded-xl hover:bg-brand-50 transition-colors">Get a Free Match</button>
          </div>
        </div>
      </main>
      <Footer />
